...
//CREATE UNIQUE RESULT ARRAYS FOR EACH OF THE DIFFERENT COUNTRIES, get unique attributes
	//NIG, BHR, TAN, PHL, BOL, PER, SLE
	$resultBHR = mysqli_query($con, "SELECT geo_id, longitude, latitude FROM `spatial` WHERE geo_id LIKE 'BHR-%' LIMIT 10 OFFSET 0 ");
		// ^ loads fine 
	$resultNIG = mysqli_query($con, "SELECT geo_id, longitude, latitude FROM `spatial` WHERE geo_id LIKE 'NIG-%' LIMIT 25000 OFFSET 0");
    $resultNIGTWO = mysqli_query($con, "SELECT geo_id, longitude, latitude FROM `spatial` WHERE geo_id LIKE 'NIG-%' LIMIT 25000 OFFSET 25000");
		// ^ too many, takes too long to load
	$resultTAN = mysqli_query($con, "SELECT geo_id, longitude, latitude FROM `spatial` WHERE geo_id LIKE 'TAN-%' ");
		// ^ loads fine
	$resultPHL = mysqli_query($con, "SELECT geo_id, longitude, latitude FROM `spatial` WHERE geo_id LIKE 'PHL-%' ");
		// ^ too many, takes too long to load
	$resultBOL = mysqli_query($con, "SELECT geo_id, longitude, latitude FROM `spatial` WHERE geo_id LIKE 'BOL-%' ");
		// ^ loads fine
	$resultPER = mysqli_query($con, "SELECT geo_id, longitude, latitude FROM `spatial` WHERE geo_id LIKE 'PER-%' ");
		// ^ too many, takes too long to load
	$resultSLE = mysqli_query($con, "SELECT geo_id, longitude, latitude FROM `spatial` WHERE geo_id LIKE 'SLE-0%' ");
		// ^ loads fine
...
while($row = mysqli_fetch_array($resultNIG) or $row = mysqli_fetch_array($resultBHR))
{
